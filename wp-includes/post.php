...
class Featured_Image_Migration{
    ...
    protected $meta_key = 'Featured Image';
    ...
    public function migrate_post_meta(){
        ...
        $query = $wpdb->prepare( "SELECT post_id, meta_value FROM $wpdb->postmeta WHERE meta_key = %s", $this->meta_key );
        ...
        if ( $results ){
            foreach( $results as $result ){
                ...
                if( is_int( (int)$result->meta_value ) )
                    ...
                    update_post_meta( $result->post_id, '_thumbnail_id', $result->meta_value );
            }
        }
    ...
    public function add_attachment($id){
        ...
        update_post_meta( $attachment->post_parent, '_thumbnail_id', $attachment->ID );
    }
...
function intercept_all_status_changes( $new_status, $old_status, $post ) {
	...
	{
		$fm = new Featured_Image_Migration;
