...
function dw_minion_comment( $comment, $args, $depth ) {
	...
	<li id="comment-<?php comment_ID(); ?>" <?php comment_class(); ?>>
		<div class="comment-body">
			<?php _e( 'Пингбек:', 'dw-minion' ); ?> <?php comment_author_link(); ?> <?php edit_comment_link( __( 'Изменить', 'dw-minion' ), '<span class="edit-link">', '</span>' ); ?>
		</div>
	...
	<li id="comment-<?php comment_ID(); ?>" <?php comment_class( empty( $args['has_children'] ) ? '' : 'parent' ); ?>>
		<article id="div-comment-<?php comment_ID(); ?>" class="comment-body">
				...
				<div class="comment-metadata">
					<a class="comment-datetime" href="<?php echo esc_url( get_comment_link( $comment->comment_ID ) ); ?>">
						...
						<time datetime="<?php comment_time( 'c' ); ?>">
							<?php printf( _x( '%1$s в %2$s', '1: date, 2: time', 'dw-minion' ), get_comment_date(), get_comment_time() ); ?>
						</time>
					</a>
					<?php edit_comment_link( __( '<i class="icon-pencil"></i> Изменить', 'dw-minion' ), '<span class="edit-link">', '</span>' ); ?>
				</div>

				<?php if ( '0' == $comment->comment_approved ) : ?>
				<p class="comment-awaiting-moderation"><?php _e( 'Комментарий ожидает проаерки', 'dw-minion' ); ?></p>
				<?php endif; ?>
			</footer>
			...
			<span class="reply">
				<?php comment_reply_link( array_merge( $args, array( 'add_below' => 'div-comment', 'reply_text' => '<i class="icon-reply"></i> Ответить' ,'depth' => $depth, 'max_depth' => $args['max_depth'] ) ) ); ?>
			</span>
		</article>
...
function dw_minion_entry_meta() {
	...
	if ( ! has_post_format('quote') ) {
		printf( __( '<span class="byline"> Автор: <span class="author vcard"><a class="url fn n" href="%1$s" title="%2$s">%3$s</a></span></span>', 'dw-minion' ),
			esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ),
			esc_attr( sprintf( __( 'Все записи автора: %s', 'dw-minion' ), get_the_author() ) ),
			esc_html( get_the_author() )
		);

		$categories_list = get_the_category_list( __( ', ', 'dw-minion' ) );
		if ( $categories_list )
			printf( __( '<span class="cat-links"> Рубрика: %1$s</span>', 'dw-minion' ), $categories_list );
	...
	if ( ! post_password_required() && comments_open() ) { ?>
		<span class="comments-link"><?php comments_popup_link( __( '<i class="icon-comment-alt"></i> Нет комментариев', 'dw-minion' ), __( '<i class="icon-comment-alt"></i> Один комментарий', 'dw-minion' ), __( '<i class="icon-comment-alt"></i> Комментариев: %', 'dw-minion' ) ); ?></span>
	<?php }
...
<?php $related_query = new wp_query( $args ); ?>
	...
	<div class="related-posts">
		<h2 class="related-posts-title"><?php _e( 'Похожие записи', 'dw_minion' ); ?></h2>
