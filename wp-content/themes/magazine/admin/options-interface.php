...
function optionsframework_tabs() {
	...
	foreach ( $options as $value ) {
		...
		if ( $value['type'] == "heading" ) {
			...
			$menu .= '<a id="options-group-'.  $counter . '-tab" class="nav-tab ' . $class .'" title="' . esc_attr( $value['name'] ) . '" href="' . esc_attr( '#options-group-'.  $counter ) . '" style="font-size:17px;">' . esc_html( $value['name'] ) . '</a>';
		}
	}
