...
class Simple_Local_Avatars {
	...
	public function __construct() {
		...
		$this->avatar_ratings = array(
			'G' => __('G &#8212; Suitable for all audiences', 'simple-local-avatar'),
			'PG' => __('PG &#8212; Possibly offensive, usually for audiences 13 and above', 'simple-local-avatar'),
			'R' => __('R &#8212; Intended for adult audiences above 17', 'simple-local-avatar'),
			'X' => __('X &#8212; Even more mature than above', 'simple-local-avatar')
		);
	...
	<table class="form-table">
		<tr>
			...
			?>
					<p style="display: inline-block; width: 26em;">
						<span class="description"><?php _e( 'Choose an image from your computer:', 'magazine' ); ?></span><br />
		...
		<tr>
			<th scope="row"><?php _e('Rating', 'magazine'); ?></th>
			<td colspan="2">
				<fieldset id="simple-local-avatar-ratings" <?php disabled( empty( $profileuser->simple_local_avatar ) ); ?>>
					<legend class="screen-reader-text"><span><?php _e('Rating', 'simple-local-avatar'); ?></span></legend>
					<?php
						...
						endforeach;
					?>
					<p class="description"><?php _e( 'If the local avatar is inappropriate for this site, Gravatar will be attempted.', 'simple-local-avatar' ); ?></p>
				</fieldset></td>
		</tr>
	</table>
	<?php
	...
	public function action_remove_simple_local_avatar() {
		if ( ! empty( $_GET['user_id'] ) &&  ! empty( $_GET['_wpnonce'] ) && wp_verify_nonce( $_GET['_wpnonce'], 'remove_simple_local_avatar_nonce' ) ) {
			...
			if ( ! current_user_can('edit_user', $user_id) )
				wp_die( __('You do not have permission to edit this user.', 'simple-local-avatar') );
