...
<div class="ratings <?php if(is_singular()) print 'hreview-aggregate'; ?>" data-post="<?php the_ID(); ?>">
  ...
  <ul <?php if(!PostRatings()->currentUserCanRate()) print 'class="rated"'; ?>  style="width:<?php print ($max_rating * 12); ?>px" title="<?php echo $current_rating; ?>">
    <li class="rating" style="width:<?php print ($rating * 12); ?>px">
      <span class="average">
    ...
    <?php if(PostRatings()->currentUserCanRate()): // only display links if the user can rate the post ?>
      ...
      <?php for($i = 1; $i <= $max_rating; $i++): ?>
        ...
        <li class="s<?php print $i; ?>">
          <a title="<?php echo $title; ?>"><?php print apply_filters('post_ratings_control_text', sprintf('%d / %d', $i, $max_rating), $i, $max_rating); ?></a>
        </li>
